...
export default function NeetCode150() {
  ...
  return (
    <MainLayout>
      ...
      {activeProblem && (() => {
        const activeIdx = NEETCODE_150_PROBLEMS.findIndex((p) => p.id === activeProblem.id);
        const hasPrev = activeIdx > 0;
        const hasNext = activeIdx >= 0 && activeIdx < NEETCODE_150_PROBLEMS.length - 1;
        ...
        return (
          <PracticeEditorModal
            ...
            sheetTitle="NeetCode 150 Roadmap"
            hasPrev={hasPrev}
            hasNext={hasNext}
            onPrevProblem={() => { if (hasPrev) setActiveProblem(NEETCODE_150_PROBLEMS[activeIdx - 1]); }}
            onNextProblem={() => { if (hasNext) setActiveProblem(NEETCODE_150_PROBLEMS[activeIdx + 1]); }}
          />
        );
      })()}
    </MainLayout>
